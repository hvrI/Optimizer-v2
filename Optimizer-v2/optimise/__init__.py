...
class Name():
    ...
    def add_change_registry(self, path: str, values: tuple):
        """
        ...
        with winreg.ConnectRegistry(None, REGISTRY_HKEY.get(path.split("\\")[0])) as hkey:
            try:
                with winreg.OpenKey(hkey, path[path.index("\\")+1:], 0, winreg.KEY_ALL_ACCESS) as rkey:
                    ...
                    for i in range(50):
                        if value_name in winreg.EnumValue(rkey, i):
                            if str(winreg.EnumValue(rkey, i)[1]) == str(value):
    ...
    def all_services_tweak(self):
        ...
        for path, values in serviceReg.items():
            for value_name, value_type, value in values:
                result = self.add_change_registry(path, (value_name, value_type, value)) # (Complete Registry Path, Key Name, Key Value, True/False)
    ...
    def antivirus_disabler(self):
        for path, values in AntivirusReg.items():
            for value_name, value_type, value in values:
                result = self.add_change_registry(path, (value_name, value_type, value)) # (Complete Registry Path, Key Name, Key Value, True) / (Complete Registry Path, False)
                if result[-1]:
                                ...
                                for i in range(winreg.QueryInfoKey(rkey)[1]):
                                    ...
                                    if value_name == retrieve_value[0] and REGISTRY_DATATYPE_REPRESENTATION.get(value_type) == retrieve_value[2]:
                                        if value == str(retrieve_value[1]):
                                            ...
                                            yield (path, keyName, str(retrieve_value[1]), True)
                                            break
                                        else:
